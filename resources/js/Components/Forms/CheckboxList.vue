...
const emit = defineEmits(['update:modelValue', 'update:selectedAll']);
...
async function updateSelectAllState(values = null) {
    if (!props.nullIfSelectedAll) {
        ignoreSleAllWatcher.value = true;

        if ((values ?? selectProxy.value)?.length === props.items.length) {
